...
## 2026-09-10: Model-Proposing Agent — Provider And Dependency

- Reused component: none. `finne/agent.py` is original code written for this project; no example, template, or SDK code was copied.
- External service: **OpenRouter** (`https://openrouter.ai/api/v1/chat/completions`), used as the model provider for the proposing agent under `DECISION-027`. Chosen because Arko holds a key for it. Called over its OpenAI-compatible REST interface with a single JSON POST; **no OpenRouter or OpenAI SDK is installed or imported.**
- Default model: `anthropic/claude-opus-5`, addressed by OpenRouter slug and overridable via `FINNE_AGENT_MODEL`. The slug and its structured-output support were verified against OpenRouter's live `/api/v1/models` endpoint on 2026-09-10 rather than assumed.
- Dependency declared: **`requests>=2.31`.** This is not a new installation — `requests` was already present in every environment as a transitive dependency of `web3` and `py-solc-x`. It is now declared explicitly so `finne/agent.py` does not rely on a dependency edge this project does not control. Stdlib `urllib` was tried first and rejected: it fails certificate verification on a machine with no default CA bundle, which was the case here. **This amends `DECISION-023`'s declared dependency list by one entry; the installed footprint is unchanged.** Licence: Apache-2.0, compatible with this repository's MIT licence.
- Third-party content: OpenRouter's structured-outputs documentation was consulted on 2026-09-10 for the exact `response_format` shape and for the caveat that schema enforcement varies by provider — the reason `provider: {"require_parameters": true}` is sent on every request. No text was copied beyond the parameter names and one short quoted phrase, attributed in `SPEC-002` section 3.
- Datasets, templates, assets: None. The opportunity description in `finne/demo_config.py` is authored for this project and describes no real protocol.
