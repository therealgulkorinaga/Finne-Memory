...
- Human-only controls: Arko approves commits, personally verifies affected product behavior, confirms understanding, and controls merge to `main`.
...
- Approval status: Governance direction approved; current file edits remain uncommitted pending Arko's review.
...
- Approval boundary: Corrections are authorized, but staging, commit, push, merge, `PREREQ-003`, and `TASK-001` authorization remain prohibited pending a clean second-pass review and later explicit approval.
