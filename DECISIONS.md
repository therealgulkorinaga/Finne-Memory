...
### DECISION-026: Add A Static Demonstration Viewer At `web/`
...
- Status: `Accepted`
- Classification: CONFIRMED
- Context: Arko designed a demonstration viewer in Claude Design (project `51086be5-d2b8-412c-ac5a-9eaf4a11b298`, artboard `Finne Memory Demo.dc.html`) and directed that it be implemented. `SPEC-001` section 15 excludes a web application from `TASK-001`'s scope while explicitly permitting one "tracked separately, outside `SPEC-001`'s authorization, if pursued" — so this is an anticipated addition, and this record is the separate tracking that permission asks for. `PREREQ-003` section 16's choice of a terminal interface is reasoning about the demo VIDEO — "two side-by-side terminals make process separation self-evident on video" — not a prohibition on other surfaces. **Correction:** Claude's first framing presented this as an architectural conflict requiring justification, which overstated it; Arko challenged that framing and re-reading the documents showed the addition was anticipated rather than contradictory. Recorded because the overstatement, not the addition, was the error.
- Decision: Add `web/`, containing a single static page (`web/index.html`) and vendored design tokens (`web/quay-tokens.css`). It renders a RECORDED run of the two-session demonstration and performs exactly one live operation: a read-only `eth_call` to the deployed `AuthorizationReceipt` contract on Base Sepolia. It reimplements no authority logic, connects no wallet, signs nothing, and computes no authorized amount. The terminal demonstration remains the primary artifact and the one recorded for the submission video.
- Rationale: The viewer is an explainer, not a second implementation. Three properties keep it from weakening the project's central claim: (1) it computes no authority — every displayed decision value is fixed recorded output, stated as such on the page itself; (2) it makes process separation MORE explicit than the terminal does, with a labelled process boundary, an explicit "Process exited" state, and a "Fresh process. No carried-over state." marker; (3) its only live element is a genuine onchain read that a judge can independently reproduce, and it fails visibly rather than falling back to cached values. A simulator that recomputed authority in JavaScript was considered and rejected: it would show a reimplementation rather than the real deterministic engine, and would be actively damaging under an event gate that tests whether memory is load-bearing.
- Consequences: `PREREQ-003` section 16's choice of a terminal interface is unchanged and remains the interface of record; this adds a surface, it does not replace one. No application module changes, no dependency is added, and `finne/` is untouched — the page is static and has no build step, so `DECISION-023`'s dependency list is unaffected. `HACKATHON_RULES.md` line 43 still requires fresh-session recall to be demonstrated in the video as one continuous unedited segment, which the terminal run satisfies and this page does not attempt to. The vendored design tokens are recorded in `REUSED_COMPONENTS.md`. Any future change that made this page compute, simulate, or transact would require a new decision.
- Human decision-maker: Arko, on 2026-09-10, having been shown the architectural conflict before implementation began.
- Related: `PREREQ-003` section 16, `SPEC-001` section 15, `REUSED_COMPONENTS.md`, `web/README.md`

### DECISION-027: Reinstate A Model At Runtime, As The Proposing Agent Only
...
- Status: `Accepted`
- Classification: CONFIRMED
- Context: Arko asked where the AI agent was in this project. There was none. `scripts/session1.py` line 87 reads `proposal = build_proposal(owner_policy.max_amount)` — the "agent" proposed the owner ceiling because the script handed it the ceiling, and no model existed anywhere in the runtime. The product's claim is that an autonomous agent will ask for more than it has earned and that Finné Memory is what bounds it; a hardcoded constant cannot overreach, so the demonstration was bounding a number the script itself chose. No event rule requires a model — the register was re-read in full to confirm that — but the event is "build with agents that don't forget", and the gap between the documents' language ("the agent proposes 25,000") and the implementation was an overclaim of exactly the kind this project has spent five review rounds removing from its code.
- Decision: Add `finne/agent.py`, a model-backed proposer, specified in `SPEC-002`. It receives an opportunity and its available capital, and produces a `Proposal`. It is never told the owner ceiling, any precedent, or any authority value. Everything downstream — comparability, derivation, the intersection engine, persistence, signing, explanation — stays deterministic and unchanged. Selection is by flag: `--agent=fixed` remains the default, `--agent=model` is opt-in.
- Provider: OpenRouter, over its OpenAI-compatible endpoint, chosen because Arko holds a key for it. Structured output is requested with a strict JSON schema **and** `provider: {"require_parameters": true}` — OpenRouter's own documentation states that schema enforcement varies by provider and that some "treat it as a strong hint", which on a recorded demo is an unrecoverable failure. No SDK is added; the call is a single JSON POST. **Corrected during implementation:** it uses `requests`, which is already installed everywhere as a transitive dependency of `web3`, and which is now declared explicitly rather than relied on implicitly — stdlib `urllib` fails certificate verification on a machine with no default CA bundle. `DECISION-023`'s declared dependency list therefore gains one entry; the installed footprint is unchanged.
- Rationale: This reverses part of the seam (e) model removal, and the reason it is safe to reverse is that the removal's finding does not apply here. What was removed was model-written PROSE displayed beside a decision, which four review rounds each found a new way to make false. What is added is a model-produced STRUCTURED PROPOSAL that the deterministic engine then bounds — the model states what it wants, not what is true. A false proposal is not a hazard; it is the input the product exists to constrain. The alternatives were rejected: leaving it as-is preserves a documented overclaim, and reframing the product as pure infrastructure was viable but forfeits the demonstration the event is built around.
- Consequences: `SPEC-001` sections 9 and 15 are amended — a model may participate in proposal generation only, never in authorization. `PREREQ-003` section 6 is narrowed to say `finne/base/` is the only module holding **signing** key material or reaching **Base**; `finne/agent.py` holds an API credential and reaches the model provider. `PREREQ-003` section 17's model-call permission moves from `finne/explain.py` — which never exercised it and remains deterministic — to `finne/agent.py`. Four invariants are added (`SPEC-002` section 10) and four acceptance criteria (A15-A18). The default path, the full test suite, and the organiser's deletion gate are unaffected: all three continue to run with no key and no network. `INV-1` continues to hold against model input — whatever is proposed, the authorized amount never exceeds the owner ceiling.
- Human decision-maker: Arko, on 2026-09-10, having identified the gap himself and chosen the provider.
- Related: `SPEC-002`, `SPEC-001` sections 9 and 15, `PREREQ-003` sections 6 and 17, `HUMAN_DECISIONS.md` 2026-09-05 (the removal this partially reverses), `REUSED_COMPONENTS.md`

### DECISION-028: Domain Pivot To Insurance Claims, And Reframing As Pre-Dispute Decision Control
...
- Status: `Accepted`
- Classification: CONFIRMED
- Context: Arko raised two connected objections to the product as framed. First, **Base was not naturally central**: forcing it into an agent-spending narrative reads as bounty engineering, and the field is crowded — CANON and Meritor already use case history to set deal terms and settle in USDC. Second, **"bounded authority for autonomous agents" has no budget line.** It is an interesting capability without an urgent buyer. The underlying engine — precedent retrieval, comparability, authority-state lifecycle, learned constraint — is unchanged by either objection; what changes is the domain it is instantiated in and the value it is sold on.
- Decision: Re-instantiate the active demonstration domain as **insurance claims assessment**, and reframe the product as a **pre-dispute decision-control layer for AI-assisted institutional workflows**. The pitch becomes "help institutions catch inconsistent decisions before customers do" rather than "help agents remember what they are allowed to spend". Base is reframed from an execution layer to a **tamper-evident attestation layer**: the `AuthorizationReceipt` anchors a hash of the decision record so it can later be proven the record existed in that form and was not rewritten once a complaint arose.
- Why the engine does not change: `PREREQ-002`'s object model has always been domain-agnostic and has now survived two pivots — supplier onboarding to agent authority (`DECISION-022`), and agent authority to claims. Five of the seven fact dimensions (`action_class`, `target_class`, `function`, `counterparty_risk_tier`, `amount`) are already neutral. The remaining two carry natural insurance meanings: `network` becomes the claims channel or panel (in-network / out-of-network is standard insurance vocabulary) and `asset` becomes the settlement currency, which is also how `finne/cli.py` already renders it. **No type-layer rename is required**, so the authority engine, memory adapter, state machine, retrieval, and Base adapter are untouched.
- What improves rather than merely transfers: the authority states finally carry their natural real-world meaning. `active` is a decision that stands as precedent; `questioned` is a decision under complaint or appeal; `superseded` is a decision made under policy wording that has since been replaced; `withdrawn` is a decision overturned by an ombudsman. In the agent domain `questioned` and `superseded` were somewhat synthetic. In claims they are the actual lifecycle, and the fact that a withdrawn decision stays retrievable but cannot authorize is exactly what an insurer needs.
- Rationale for the commercial reframing: a dispute typically arises from one of four causes — two similar cases treated differently, a decision resting on outdated policy, an institution unable to reconstruct why a decision was made, or an exception applied inconsistently. The existing engine attacks all four directly, and each has a quantifiable cost of inaction (complaint handling, ombudsman fees, remediation, regulatory scrutiny, churn) that the agent-permission framing did not.
- Consequences: `docs/product/ACTIVE_DEMO_DESIGN.md`'s corpus is replaced with eight claim precedents. `config/owner_policy.toml` becomes a claims handler's delegated settlement authority. `finne/demo_config.py`, `scripts/reset_demo.py`, and `finne/agent.py`'s prompt change to match. Roughly four test files that assert the old domain's values are updated; the rest build their own fixtures and are unaffected. `README.md`, `PRD.md`, `ARCHITECTURE.md`, `PREREQ-003` and `SPEC-001`/`SPEC-002` are reframed. **The demonstration's numbers, mechanics, and the organiser's memory-deletion gate are unchanged** — a fresh session still retrieves precedent, still derives a learned constraint, and still cannot act when the memory is removed.
- Explicitly not claimed: the insurance scenario is illustrative and is built on synthetic fixtures. `HACKATHON_RULES.md` line 62 requires PMF evidence to be publicly verifiable and states that fabricated evidence disqualifies. No pilot, design partner, or validated insurer engagement is claimed anywhere, and the corpus must never be presented as real claims data.
- Human decision-maker: Arko, on 2026-09-10, who identified both the Base-centrality problem and the dispute-prevention framing himself.
- Related: `DECISION-022` (the previous domain pivot, same mechanism), `DECISION-026`, `DECISION-027`, `docs/product/ACTIVE_DEMO_DESIGN.md`, `HACKATHON_RULES.md` line 62
