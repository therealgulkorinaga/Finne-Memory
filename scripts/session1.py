...
def run(db_path: Path, owner_approved_amount: Decimal, agent_mode: str = "fixed") -> int:
...
def main() -> None:
    ...
    sys.exit(
        run(Path(args.db_path).expanduser(), Decimal(args.owner_approved_amount), args.agent)
    )
